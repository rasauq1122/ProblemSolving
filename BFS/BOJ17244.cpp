...
int vis[1<<5][50][50], arr[50][50], sx, sy;
...
void bfs() {
    ...
    while (!q.empty()) {
        ...
        rep(i, 0, 4) {
            ...
            if (!safe(xx,yy,n,m)) continue;
            if (arr[xx][yy] == -1) continue;
            if (1 <= arr[xx][yy]) kkey |= 1<<arr[xx][yy]-1;
            if (vis[kkey][xx][yy] && vis[kkey][xx][yy] <= 1 + vis[key][x][y]) continue;
            vis[kkey][xx][yy] = 1 + vis[key][x][y];
            q.push({kkey,{xx,yy}}); 
        }
    }
}
...
signed main() {
    ...
    rep(i, 0, n) {
        scs(s);
        rep(j, 0, m) {
            if (s[j] == 'X') arr[i][j] = ++cnt;
            else if (s[j] == '#') arr[i][j] = -1;
            else if (s[j] == 'E') arr[i][j] = -2;
            else if (s[j] == 'S') sx = i, sy = j;
        }
    }
    bfs();
    rep(i, 0, n) rep(j, 0, m) if (arr[i][j] == -2) cout << vis[(1<<cnt)-1][i][j]-1;
}