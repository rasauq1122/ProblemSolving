...
signed main() {
    ...
    rep(i, 1, n+1) cin >> r[i];
    rep(i, 1, n+1) cin >> c[i];
    rep(i, 1, n+1) r[i] = r[i]%2, c[i] = c[i]%2;
    rep(i, 1, n) r[i] = (r[i] != r[i+1]), c[i] = (c[i] != c[i+1]);
    rep(i, 1, n) r[i] += r[i-1], c[i] += c[i-1];
    rep(i, 0, m) {
        int r1, c1, r2, c2;
        cin >> r1 >> c1 >> r2 >> c2;
        int dr = max(r1,r2), dc = max(c1,c2), sr = min(r1,r2), sc = min(c1,c2);
        if ((r[dr-1]-r[sr-1])||(c[dc-1]-c[sc-1])) cout << "NO" << endl;
        else cout << "YES" << endl;
    }
}