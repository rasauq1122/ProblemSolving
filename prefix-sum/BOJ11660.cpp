...
int n, m, k, arr[1025][1025], ps[1025][1025];

signed main() {
    IOSFAST();
    cin >> n >> m;
    rep(i, 0, n) rep(j, 0, n) cin >> arr[i+1][j+1];
    rep(i, 1, n+1) rep(j, 1, n+1) ps[i][j] += arr[i][j] + ps[i][j-1];
    rep(j, 1, n+1) rep(i, 1, n+1) ps[i][j] += ps[i-1][j];
    rep(i, 0, m) {
        int x1, x2, y1, y2;
        cin >> x1 >> y1 >> x2 >> y2;
