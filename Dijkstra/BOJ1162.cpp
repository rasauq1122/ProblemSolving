...
signed main() {
    ...
    while (!pq.empty()) {
        int cost = pq.top().ff;
        int vtx = pq.top().ss.ff;
        int po = pq.top().ss.ss;
        
        pq.pop();
        if (dist[vtx][po] != -1) continue;
        dist[vtx][po] = cost;
        rep(i, 0, v[vtx].size()) {
            pii nxt = v[vtx][i];
            if (dist[nxt.ff][po] == -1) pq.push({cost+nxt.ss, {nxt.ff,po}});
            if (po > 0 && dist[nxt.ff][po-1] == -1) pq.push({cost, {nxt.ff,po-1}});
        }
    }
    repc(i, 0, k) so = min(so, dist[n][i]);
    cout << so;
}