...
W(int) w, arr;
int cap[2502][2502], flo[2502][2502];
...
signed main() {
    ...
    rep(i, 0, n) {
        ssc(s);
        rep(j, 0, m) arr[i][j] = s[j] == 'X', cnt -= arr[i][j];
    }
    ...
    rep(i, 0, n) rep(j, 0, m) {
        int now = i*m+j+1;
        if (arr[i][j]) continue;
        if ((i+j)%2) w[now].pb(en), w[en].pb(now), cap[now][en] = 1;
        else {
            w[st].pb(now), w[now].pb(st), cap[st][now] = 1;
            rep(k, 0, 4) {
                int x = i + dx[k], y = j + dy[k];
                if (!safe(x,y,n,m) || arr[x][y]) continue;
                int nxt = x*m+y+1;
                w[now].pb(nxt), w[nxt].pb(now), cap[now][nxt] = 1;
            }
        }
    }
    pf(cnt - flow(st,en));
