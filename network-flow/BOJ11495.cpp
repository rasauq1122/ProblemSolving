...
int flow(int st, int en) {
    ...
    loop() {
        ...
        while(q.size() && v[en] == -1) {
            ...
            rep(i, 0, w[now].size()) {
                int nxt = w[now][i];
                if (v[nxt] == -1 && c[now][nxt] > f[now][nxt]) {
                    v[nxt] = now;
                    q.push(nxt);
                }
            }
        ...
        so = IMAX;
        for(int i = en ; i != st ; i = v[i]) so = min(so, c[v[i]][i]-f[v[i]][i]);
        for(int i = en ; i != st ; i = v[i]) f[v[i]][i] += so, f[i][v[i]] -= so;
        rtn += so;
    }
    return rtn;
}
...
signed main() {
    ...
    TC() {
        rd2(n, m);
        arr = vi2(n,m), c = vi2(n*m+2,n*m+2), f = vi2(n*m+2,n*m+2);
        w = wi(n*m+2);
        sum = 0;
        rep(i, 0, n) rep(j, 0, m) rd(arr[i][j]), sum += arr[i][j];
        int st = 0, en = n*m+1;
        rep(i, 0, n) rep(j, 0, m) {
            int now = i*m+j+1;
            if ((i+j)%2) w[now].pb(en), c[now][en] = arr[i][j], w[en].pb(now);
            else {
                w[st].pb(now), c[st][now] = arr[i][j], w[now].pb(st);
                rep(k, 0, 4) {
                    int x = i + dx[k], y = j + dy[k];
                    if (!safe(x,y,n,m)) continue;
                    int nxt = x*m+y+1;
                    w[now].pb(nxt), c[now][nxt] = arr[i][j], w[nxt].pb(now);    
                }
            }
        }
        pfl(sum-flow(st,en));
