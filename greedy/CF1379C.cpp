...
signed main() {
    ...
    TC() {
        ...
        rep(i, 0, m) {
            int x;
            rd2(v[i], x), w.pb({v[i],x});
        }
        ST(v);
        per(i, 0, m) ps.pb(ps.back()+v[i]);
        da = 0;
        rep(i, 0, m) {
            int k = ub(all(v), w[i].ss) - v.begin();
            if (w[i].ss < w[i].ff) da = max(da, ps[min(m-k,n)]+(n-min(m-k,n))*w[i].ss);
            else da = max(da, ps[min(m-k,n-1)]+(n-1-min(m-k,n-1))*w[i].ss+w[i].ff);
        }
        pf1(da);
    }   
}