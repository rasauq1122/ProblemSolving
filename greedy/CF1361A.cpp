...
signed main() {
    ...
    while(!pq.empty()) {
        pii now = pq.top();
        V(bool) arr(v[now.ss].size()+2);
        rep(i, 0, v[now.ss].size()) if (vis[v[now.ss][i]] <= v[now.ss].size()+1) arr[vis[v[now.ss][i]]] = 1;
        repc(i, 1, v[now.ss].size()+1) if (!arr[i]) {
            vis[now.ss] = i;
            break;
        }
        if (vis[now.ss] != now.ff) return cout << -1, 0;
        pq.pop();
        q.push(now.ss);
    }

    for (; !q.empty() ; q.pop()) cout << q.front() << blk;
