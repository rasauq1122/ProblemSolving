...
signed main() {
    ...
    V(pii) y;
    rep(i, 0, n) cin >> v[i]; 
    rep(i, 0, n) rep(j, i+1, n) w.pb({v[i]+v[j],{i,j}});
    ST(w);
    rep(i, 0, w.size()) x.pb(w[i].ff), y.pb(w[i].ss);
    rep(i, 0, n) rep(j, i+1, n) {
        int st = lb(x, m-(v[i]+v[j])), en = ub(x, m-(v[i]+v[j]));
        if (x[st] != m-(v[i]+v[j])) continue;
        rep(k, st, en) if (!(y[k].ff == i || y[k].ff == j || y[k].ss == i || y[k].ss == j)) return cout << "YES", 0;
    }
    cout << "NO";
}