...
int fast(int a, int b, int c) {
    if (b == 0) return 1;
    int val = fast(a,b/2,c);
    val *= val, val %= c;
    if (b%2) val *= a, val %= c;
    return val;
}

int get(int a, int r, int n, int md) {
    if (n == 1) return a % md;
    if (n%2) {
        int k = ((r % md) * get(a, r, n/2, md)) % md;
        return ((((1+fast(r,n/2,md)) * k) % md) + a) % md;
    } else {
        int k = (get(a, r, n/2, md)) % md;
        return ((1+fast(r,n/2,md)) * k) % md;
    }
}
