...
W(int) add(W(int) a, W(int) b) {
    W(int) c(n, V(int)(n));
    rep(i, 0, n) rep(j, 0, n) c[i][j] += (a[i][j]+b[i][j])%BIG;
    return c;
}

W(W(int)) mult2(W(W(int)) a, W(W(int)) b) {
    W(W(int)) c(5, V(W(int))(5, W(int)(n, V(int)(n)))); 
    rep(i, 0, 5) rep(j, 0, 5) rep(k, 0, 5) c[i][j] = add(c[i][j], mult(a[i][k], b[k][j]));
    return c;
}

W(W(int)) ppow(W(W(int)) a, int b) {
    if (!b) {
        W(W(int)) c(5, V(W(int))(5, W(int)(n, V(int)(n))));
        rep(i, 0, 5) rep(j, 0, n) c[i][i][j][j] = 1;
        return c;
    }
    if (b%2) return mult2(a, ppow(a,b-1));
    W(W(int)) c = ppow(a, b/2);
    return mult2(c, c);
}
...
signed main() {
    ...
    W(W(int)) a(5, V(W(int))(5, W(int)(n, V(int)(n))));
    rep(i, 0, 4) rep(j, 0, n) a[i+1][i][j][j] = 1;
    rep(i, 0, n) rep(j, 0, n) if (arr[i][j]) a[0][arr[i][j]-1][i][j] = 1;
    a = ppow(a,k);
    
    cout << a[0][0][st-1][en-1];
}