...
int vis[10001], scc[10001], cnt2;
V(V(int)) v(10001), ans(10001);
S(int) s; 
...
signed main() {
    ...
    repc(i, 1, n) if (!vis[i]) dfs(i);
    repc(i, 1, n) ans[scc[i]].pb(i);
    sort(ans.begin()+1, ans.begin()+cnt2+1);
    cout << cnt2 << endl;
    repc(i, 1, cnt2) {
        rep(j, 0, ans[i].size()) cout << ans[i][j] << blk;
        cout << "-1" << endl;
    }
}