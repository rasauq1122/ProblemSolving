...
int vis[100001], scc[100001], cnt2, in[100001];
V(V(int)) v;
S(int) s; 
...
signed main() {
    ...
    TC() {
        ...
        rep(i, 0, n) rep(j, 0, v[i].size()) if (scc[i] != scc[v[i][j]] && !in[scc[v[i][j]]]) in[scc[v[i][j]]] = 1, cnt2--;
        if (cnt2 != 1) cout << "Confused" << endl;
        else rep(i, 0, n) if (!in[scc[i]]) cout << i << endl;
        cout << endl;
    }
}