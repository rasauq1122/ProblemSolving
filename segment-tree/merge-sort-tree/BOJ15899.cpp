...
void dfs(int idx, int p) {
    sub[idx][0] = ++cnt;
    w[h+cnt-1].pb(v[idx]);
    rep(i, 0, eg[idx].size()) if (eg[idx][i] != p) dfs(eg[idx][i], idx);
    sub[idx][1] = cnt;
}
...
signed main() {
    ...
    rd3(n,m,l);
    while (h < n) h <<= 1;
    w = wi(2*h), eg = wi(n+1), v = vi(n+1), sub = vi2(n+1,2); 
    repc(i, 1, n) cin >> v[i];
    rep(i, 1, n) {
        sc2(x,y), eg[x].pb(y), eg[y].pb(x);
    }
    dfs(1, 0);
    rep(i, n, h) w[h+i].pb(IMAX); // 쓰레기값 넣기
    ...
    rep(i, 0, m) {
        sc2(a, b);
        sum += sub[a][1]-sub[a][0]+1-get(sub[a][0],sub[a][1],1,h,b,1);
        sum %= MOD;
    }
    pf(sum);
}