...
bool loop = false;
int arr[50][50], vis[50][50], dp[50][50], dx[4] = {1,-1,0,0}, dy[4] = {0,0,1,-1};

int dfs(int x, int y) {
    if (x < 0 || y < 0 || x >= n || y >= m || arr[x][y] == -1) return 0;
    if (loop) return n*m;
    if (vis[x][y]) {
        loop = true;
        return n*m;
    ...
    vis[x][y] = 1;
    int da = 0;
    rep(i, 0, 4) da = max(da, dfs(x+dx[i]*arr[x][y],y+dy[i]*arr[x][y]));
    vis[x][y] = 0; 
    return dp[x][y] = 1 + da;
}
...
signed main() {
    ...
    rep(i, 0, n) {
        scs(s);
        rep(j, 0, m) arr[i][j] = (s[j] == 'H' ? -1 : s[j]-'0');
    }
    MS(dp);
    k = dfs(0, 0);
    if (k > n*m) cout << -1;
