...
void dfs(int i, int p) {
    rep(j, 0, v[i].size()) if (v[i][j] != p) w[i].pb(v[i][j]), dfs(v[i][j], i);
}

int foo(int i, bool c) {
    if (!w[i].size()) return c;
    if (dp[i][c] != -1) return dp[i][c];
    int sum = c;
    if (c) rep(j, 0, w[i].size()) sum += min(foo(w[i][j],0), foo(w[i][j],1));
    else rep(j, 0, w[i].size()) sum += foo(w[i][j],1);
    return dp[i][c] = sum;
}
