...
int foo(int bm, int k) {
    int cnt = 0;
    per(i, 0, n) cnt += BIT(bm, i); 
    if (cnt == 1) return dp[bm][k] = bm == (1<<k);
    if (dp[bm][k] != -1) return dp[bm][k];

    int sum = 0;
    rep(i, 0, n) if (BIT(bm,i) && abs(arr[i]-arr[k]) > m) sum += foo(bm-(1<<k), i);
    return dp[bm][k] = sum;
}
