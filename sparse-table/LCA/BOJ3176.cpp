...
int sp[100001][3][18], dep[100001];
W(pii) v(100001);

void dfs(int i, int p, int d) {
    sp[i][0][0] = p;    
    dep[i] = d;
    rep(j, 0, v[i].size()) if (v[i][j].ff != p) {
        sp[v[i][j].ff][1][0] = sp[v[i][j].ff][2][0] = v[i][j].ss; 
        dfs(v[i][j].ff, i, d+1);
    }
}
...
signed main() {
    ...
    rep(i, 1, n) {
        sc3(x,y,c);
        v[x].pb({y,c}), v[y].pb({x,c});
    }

    dfs(1, 1, 1);
    sp[1][2][0] = IMAX;
    rep(k, 1, 18) repc(i, 1, n) sp[i][0][k] = sp[sp[i][0][k-1]][0][k-1];
    rep(k, 1, 18) repc(i, 1, n) sp[i][1][k] = max(sp[i][1][k-1], sp[sp[i][0][k-1]][1][k-1]);
    rep(k, 1, 18) repc(i, 1, n) sp[i][2][k] = min(sp[i][2][k-1], sp[sp[i][0][k-1]][2][k-1]);
    ...
    TC() {
        ...
        int gap = dep[y] - dep[x];
        per(i, 0, 18) if (BIT(gap,i)) da = max(da, sp[y][1][i]), so = min(so, sp[y][2][i]), y = sp[y][0][i];
        if (x == y) {
            cout << so << blk << da << endl;
            continue;
        }
        per(i, 0, 18) if (sp[x][0][i] != sp[y][0][i]) {
            da = max({da,sp[x][1][i],sp[y][1][i]});
            so = min({so,sp[x][2][i],sp[y][2][i]});
            x = sp[x][0][i], y = sp[y][0][i];
        }
        da = max({da,sp[x][1][0],sp[y][1][0]});
        so = min({so,sp[x][2][0],sp[y][2][0]});
        cout << so << blk << da << endl;
    }
}